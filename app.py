...
# 注入 CSS：彻底修复图标字体被覆盖的 Bug，精准独立应用黑体
st.markdown(f"""
    <style>
        /* 1. 基础容器应用常规字体 */
        .stApp {{
...
if st.session_state.page == 'splash':
    ...
    st.markdown("<br><p style='text-align: center; color: #888888;'>正在加载因果森林预测引擎与多目标运筹底座...</p>",
                unsafe_allow_html=True)
    time.sleep(2.0)
...
# ------------------------------------------
    ...
    with col2:
        st.subheader("分析： 量化数值结构分析")
        st.info(
            f"通过高维极差空间拓扑计算，本系统对目标城市进行了宏观数据的降维测度。结果显示，该市在“浅层民用网络（宽带普及）”维度的标准化得分达到 {city_scores[3]:.1f}，已接近全省平均发展基准水位，这表明基于消费端的数字化红利已充分释放并趋于饱和边界。然而，系统侦测到，在直接决定宏观实体产业深度重构的核心要素指标上（如深层传输光缆与高精尖人才密度），该市数据曲线呈现出结构性落差与底层资产相对不足。")
...
# ------------------------------------------
    ...
    if current_engine_logic == "综合型":
        ...
        with col_t:
            st.subheader("分析： 深度预测与量化数值分析")
            st.info(
                f"当前底层因果森林引擎（OrthoIV）已锁定目标并自动切换为泛基准综合型城市防过热推演模式。算法在剔除了宏观多维混淆变量后确证：此类城市在现阶段缺乏可供深度消纳庞大冗余算力的重化工产业链矩阵。结合控制台实时拟定的 {st.session_state.c_invest:.1f} 亿元 资本投入，模型捕捉到了宏观不协调信号。在缺乏实体应用场景转化的约束下，该笔超前投资极易诱发明显的规模耗能效应，伴随显著的高碳代价风险。如左侧仪表盘所示，风险指数正随投资总额的攀升而逐步逼近临界过热区间。")

            st.subheader("建议： 跨部门协同防御与投资阻断建议")
            st.success(
                "前置合规性审查建议：事前仿真测算结果发出了结构性风险信号。针对规划方案中隐含的规模耗能反噬风险，相关合规部门宜在基础设施项目的立项核准阶段，加强效益比对。合理调减缺乏真实场景场景支撑的超前重资产算力或冗余光缆线路，从源头上防范固定资产低效扩张。\n\n"
        ...
        with col_t:
            st.subheader("分析： 深度预测与量化数值分析")
            st.info(
                f"基于该市的产业图谱，当前分析引擎处于典型重工业城市碳锁定破局验证模式。经过数理因果逻辑链条推演，本系统确证：数字化基础设施要素一旦成功贯穿并嵌套入高耗能生产流水线，其所爆发的能效协同优化能够表现出显著的节能对冲效应（边际增碳规模被有效控制在较低水平）。依托控制台实时的 {total_invest:.1f} 亿元 总投资规模推演，模型动态研判当前资金规模能够跨越能耗爬坡阶段、全面发挥规模报酬红利的理论概率定格为 {breakthrough_score:.1f}%。")

            st.subheader("建议： 战略定力保持与精准爆破建议")
            st.success(
                "宏观战略统筹指导：若左侧仪表盘指针徘徊于拐点蓄力区间，表明城市转型正深陷基建土木期所伴随的能耗爬坡阶段。决策层需保持较强的宏观转型定力与政策连贯性，避免因短期局部耗能反弹而产生政策摇摆。应当保障持续、稳健的资金接续机制，使之顺利跨越边际效益拐点，确保整体数字化改造项目顺利释放节能成效。\n\n"
...
# ------------------------------------------
    ...
    st.subheader("分析： 前置仿真与量化数值逆向解构分析")
    st.info(
        "依托后台封装的正交因果森林与GBM引擎，系统赋予了决策者政策试错的能力。用户可通过左侧控制台，动态拖拽各项数字基建与外资引导资金的预算滑块，系统将在毫秒级延迟内，生成实施该干预政策后的未来三年碳排放轨迹预测（如左侧折线图所示），并同步输出SHAP瀑布图。这种动态映射机制，使得各项宏观规划在正式下发前，都能在数字沙盘中进行充分的压力测试，从源头上防范决策偏差带来的沉没成本。")

    st.subheader("建议： 财税统筹与绩效审查优化核心建议")
    st.success(
        "构建基于边际效费比的财政绩效评价体系：相关部门可参考右侧量化瀑布归因，优化公共财政资金的配置结构，打破传统预算编制中的固化配置模式。在压力测试环节中呈现出推高能耗特征的重资产配置方案，应执行严格的预算压降与资金拦截评估，避免财政资源的低效错配。\n\n"
        "实施核心转型要素的乘数放大战略：在优化低效投入的同时，各相关职能部门应依托轨迹预测建立深度预审机制。在下达阶段性减排考核任务时，务必协同地方金融资源，为那些在瀑布图谱中呈现为显著负向效益（即具备长效降碳与稳增长双重红利）的优质要素渠道，统筹调配绿色转型专项转移支付，提供长期稳固的政策支撑，保障区域经济平稳过渡至高质量发展阶段。")
