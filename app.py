...
if st.session_state.page == 'splash':
    ...
    with col_main:
        with st.container(border=True):
            ...
            if cat_choice == "内置：重工锁定型城市":
                ...
                st.session_state.custom_data["基准碳排"] = c2.number_input("期初基准碳排总量(Mt)", value=float(st.session_state.custom_data["基准碳排"]))
    ...
    with st.sidebar:
        ...
        new_cat = st.radio("一、底座数据类型热切换", cat_opts, index=cat_opts.index(st.session_state.city_category.replace("宏观数据", "数据")), key="sb_cat")
        if new_cat != "外部：自定义录入数据":
            ...
            new_logic = st.radio("核定底层算法基盘", ["偏向重化工业主导", "偏向综合与服务型"], index=0 if "重化工" in st.session_state.custom_logic else 1)
