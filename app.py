...
if st.session_state.page == 'menu':
    ...
    with st.container(border=True):
        ...
        st.markdown("在经济、生态与财政多约束的宏观治理环境中，运用第三代遗传进化算法进行高维解空间探索。自动生成兼顾宏观经济拉动与碳排放收敛的结构化投资决策建议，科学引导产业高质量发展。")
...
# ------------------------------------------
    ...
    with col2:
        st.subheader("一、 量化数值结构分析")
        st.info(f"通过高维极差空间拓扑计算，本系统对目标城市进行了宏观数据的降维测度。结果显示，该市在“浅层民用网络（宽带普及）”维度的标准化得分达到 {city_scores[3]:.1f}，已接近甚至超越全省平均发展基准水位，这表明基于消费端的数字化红利已充分释放并趋于饱和边界。然而，系统侦测到，在直接决定宏观实体产业深度重构的核心要素指标上（如深层传输光缆与高精尖人才密度），该市数据曲线呈现出结构性落差与底层资产相对不足。")
        
        st.subheader("二、 统筹指导发展与精准投资建议")
        if current_engine_logic == "重化工":
            st.success("基于上述诊断，系统为重化工业主导的产业底色输出以下多部门协同指导建议：\n\n"
                        "相关规划部门：应即刻调整下一年度的财政支出分配结构。鉴于浅层网络覆盖率基本平稳，建议合理控制对消费级通信基站的边际投资增量。联合启动预算倾斜机制，优先将存量专项资金用于填补深层工业光缆的投资历史欠账，规避未来重大智造项目因算力传输瓶颈而引发的结构性制约。\n\n"
                        "工业主管体系：需主导缓解重资产企业内部的信息孤岛现象。针对具有高耗能特征的龙头企业，设立专门的工业互联网技改前置引育类目。加速数据要素向洗煤、焦化、冶金等核心排放流水线的全流程渗透，为承载高维降碳高阶应用构筑坚实的物理实体网络基座。")
        else:
            st.success("基于综合型城市的产业演进规律，系统输出以下统筹指导建议：\n\n"
                        "招商与产业管理部门：诊断预警表明，该市虽已具备初级的硬件承载力，但软件与技术生态转化效能存在上升空间，面临固定资产利用率偏低的风险。建议相关职能部门果断将中长期投资重心由单纯的硬基建扩张转向高规格技术外资与技术专利引入，着力打通数据要素向本地高端服务业深层融合的长效机制。\n\n"
...
# ------------------------------------------
    ...
    if current_engine_logic == "综合型":
        ...
        with col_t:
            ...
            st.info(f"当前底层因果森林引擎（OrthoIV）已锁定目标并自动切换为泛基准综合型城市防过热推演模式。算法在剔除了宏观多维混淆变量后确证：此类城市在现阶段缺乏可供深度消纳庞大冗余算力的重化工产业链矩阵。结合左侧控制台实时拟定的 {st.session_state.c_invest:.1f} 亿元 资本投入，模型极其敏锐地捕捉到了宏观不协调信号。在缺乏实体应用场景转化的约束下，该笔超前投资极易诱发明显的规模耗能效应，伴随显著的高碳代价风险。如左侧仪表盘所示，风险指数正随投资总额的攀升而逐步逼近临界过热区间。")
            
            st.subheader("二、 跨部门协同防御与投资阻断建议")
            st.success("前置合规性审查建议：事前仿真测算结果发出了不容忽视的结构性风险信号。针对规划方案中隐含的规模耗能反噬风险，发改等前置合规部门宜在基础设施项目的立项核准阶段，加强严格的效益比对。合理调减缺乏真实场景消化支撑的超前重资产算力或冗余光缆线路，从源头上防范固定资产低效扩张。\n\n"
                        "财政与生态环境部门：财政部门宜参考仿真阈值，建立专项债及财政支出的动态反馈调控机制，合理收紧涉边际能耗偏高项目的资金敞口，将财政资源有序导向软件生态开发与人力资本增量路径，维护地方公共杠杆率处于健康范围。同时，生态环境部门可将本模块生成的低碳边界效应指标正式纳入区域性环境影响评估，保障全域绿色发展路径与总体碳排放约束的良性协同。")
        ...
        with col_t:
            ...
            st.success("市级宏观战略统筹指导：若左侧仪表盘指针徘徊于拐点蓄力区间，表明城市转型正深陷基建土木期所伴随的能耗爬坡阶段。决策层需保持较强的宏观转型定力与政策连贯性，避免因短期局部耗能反弹而产生政策摇摆。应当保障持续、稳健的资金接续机制，使之顺利跨越边际效益拐点，确保整体数字化改造项目顺利释放节能成效。\n\n"
                        "工信部门专项干预部署：为防范政策实施过程中出现的资金碎片化与边际效力递减，工信与财政部门宜构建高度集中的技术技改补贴机制。确保公共资源形成规模合力，靶向聚焦工业车辆调度、高炉优化与绿色原材料开采等核心高能耗、高排放节点实施流程重塑与精准引导，全面释放深层数智化转型释放的长期长期边际效应。")
...
# ------------------------------------------
    ...
    st.subheader("一、 前置仿真与量化数值逆向解构分析")
    st.info(f"本智能体基于博弈论特征剥离机制（SHAP），在保持数据链严密性的基础上完成了多要素边际效费比的解构。系统预测显示：若地方政府维持历史轨迹的长期依赖，该市年度基准碳排总量将保持在 {base_carbon:.2f} Mt 左右；而一旦执行控制台拟定的综合政策投资干预案，全市中长期碳排演化轨迹将产生结构性收敛，修正至动态值 {pred_carbon:.2f} Mt。右侧的特征瀑布图以严谨的数理回归结构，量化剖析了“深层光缆铺设、高质量外资引导、高端人才引育”三大要素工具在这一碳排落差中所创造的绝对边际效费比与最终效用方向。")
    
    st.subheader("二、 财税统筹与绩效审查优化核心建议")
    st.success("构建基于边际效费比的财政绩效评价体系：财政与审计监察等部门可参考此量化瀑布归因，优化公共财政资金的配置结构，打破传统预算编制中主观经验驱动与均摊式配置的相对单一模式。针对图谱中呈现为正向边际能耗的冗余项目，执行严格的预算绩效评价与资金调配优化，防范边际能耗过高项目的资金错配。\n\n"
                "实施核心转型要素的乘数放大杠杆：在优化低效要素投入的同时，生态环境、发改与财政等部门应建立深度联席预审机制。在下达各周期阶段性减排考核任务时，务必协同地方金融监管力量，为那些在瀑布图谱中呈现为显著负向效益（即具备高效降碳与稳增长双重回报）的超优质要素渠道，统筹调配绿色转型专项转移支付，提供长期稳固的信用与政策支撑。确保极其稀缺的地方存量公共资源，全面向高效率、精准化方向跃升。")
    
    bottom_navigation('mod3')

# ------------------------------------------
# 页面 6：模块四 (多约束帕累托决策指导生成)
# ------------------------------------------
    ...
    st.subheader("二、 宏观政策决策建议与发展统筹指导意见")
    if total_invest < 4.0:
        ...
        st.error("【系统自动定性识别：高危扩张型过载策略】\n\n"
                    ...
                    "宏观发展破局指导规划：建议审计、统筹与综合协调部门启动绩效评估程序。应当通过严密的合规性测算，合理调减高能耗、超前过热的基础建设置信标段，严格肃清缺乏深入环评与消化需求论证的项目。通过科学优化举债和财政投资大盘，将总干预预算维持在安全财政容忍度内，稳步优化地方宏观资产负债表的长期健康底层韧性。")
    else:
        st.success("【系统自动定性识别：效率均衡型高质量稳步提质战略】\n\n"
                    "深度政策诊断模型研判：系统给予该项配置良性的战略评价。该资金配置方案较为精准地落入帕累托最优沿的效率有效均衡区间。在保持相对理性的投资规模下，避免了资本无序扩张，通过多要素靶向分配机制，在多目标刚性约束下，创造性地实现了宏观经济拉动与碳排放收敛在边际层面的良性协同。\n\n"
                    "宏观发展破局指导规划：该配置具有较强的现实参照意义。建议由宏观决策与办公统筹部门提取该项参数，将其作为后续编制地方中长期施政规划与年度经济社会发展报告的量化参照。引导发改、财政与工信等相关部门依据此比例关系设定项目库白名单，固化协同优势，科学指导本区域核心产业体系逐步向更具效能的低碳循环经济业态进行高质量、可持续的结构性升维。")
